...
public interface QnaBoardRepository extends JpaRepository<QnaBoardEntity, Integer> {
    ...
    "Q.writer_profile_image_url," +
    "(select count(*) from qna_comment C) AS qnaCommentCount," +
    "(select count(*) from qna_heart H) AS qnaHeartCount," +
    "(select count(*) from qna_hashtag T) AS qnaHashtagCount" +
    ...
    "AND Q.qna_board_number = T.qna_board_number",
    // group by?, order by?
    nativeQuery = true // sql문법그대로 jpa에서 쓸수 있게 해주는거
    )
